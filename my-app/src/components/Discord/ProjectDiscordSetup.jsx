...
import { FaDiscord, FaUsers, FaCog, FaSpinner, FaExternalLinkAlt } from 'react-icons/fa';
...
const ProjectDiscordSetup = ({ project, isCreator, userHasDiscord }) => {
  ...
  const setupDiscordIntegration = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/projects/${project.slug}/setup-discord`, {
        method: 'POST',
        headers: {
    ...
    return (
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <div className="flex items-start space-x-3">
          <FaDiscord className="w-5 h-5 text-green-600 mt-0.5" />
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <p className="text-sm text-blue-800">
              {userHasDiscord 
                ? "Ready for Discord! Ask the project creator to set up Discord integration." 
    ...
    if (setupResult.success) {
      ...
      return (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <div className="flex items-start space-x-3">
            <FaDiscord className="w-5 h-5 text-red-600 mt-0.5" />
            <div>
  ...
  return (
    <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
      <div className="flex items-start space-x-3">
        <FaDiscord className="w-5 h-5 text-indigo-600 mt-0.5" />
        <div className="flex-1">
