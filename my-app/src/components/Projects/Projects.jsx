...
const Projects = () => {
  ...
  const ProjectCard = ({ project, isOwned, isCompleted = false, completeProject, completingProject }) => (
    <motion.div
      ...
      className={`bg-white/20 sm:bg-white/10 backdrop-blur-md sm:backdrop-blur-sm rounded-xl border border-white/30 sm:border-white/20 p-4 sm:p-6 hover:bg-white/25 sm:hover:bg-white/15 transition-colors flex flex-col h-full shadow-lg sm:shadow-none ${
        ...
        <div className="pt-2 mt-auto">
          {isCompleted ? (
            ...
            <Link
              ...
              className="w-full bg-slate-600 hover:bg-slate-700 text-white font-medium py-2 px-4 rounded-lg flex items-center justify-center gap-2 transition-colors"
            >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 pt-20 pb-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6 sm:mb-8">
          <div>
            <h1 className="text-2xl sm:text-4xl font-bold text-white mb-2">My Projects</h1>
            <p className="text-purple-200 text-sm sm:text-base">Manage your projects and collaborations</p>
          </div>
        </div>

        {/* Tabs */}
        <div className="flex flex-wrap gap-1 bg-white/20 sm:bg-white/10 backdrop-blur-md sm:backdrop-blur-sm rounded-lg p-1 mb-6 sm:mb-8 w-full sm:w-fit shadow-lg sm:shadow-none border border-white/30 sm:border-white/20">
          <button
            onClick={() => setActiveTab('owned')}
            className={`px-3 py-2 sm:px-4 sm:py-2 rounded-md font-medium transition-colors text-xs sm:text-sm flex-1 sm:flex-none ${
              activeTab === 'owned'
                ? 'bg-purple-600 text-white'
                : 'text-purple-200 hover:text-white'
            }`}
          >
            <span className="hidden sm:inline">My Projects </span>({ownedProjects.length})
          </button>
          <button
          ...
          >
            <span className="hidden sm:inline">Participating </span>({participatingProjects.length})
          </button>
          <button
