      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className={`md:hidden fixed top-16 right-0 h-[calc(100vh-4rem)] w-80 max-w-[80vw] z-50 ${
              isProfileRoute 
                ? 'bg-black border-l border-white/20' 
                : 'bg-white border-l border-gray-200'
            } shadow-xl overflow-y-auto`}
          >
            <div className="p-6 h-full pt-4">
              {/* Close Button */}
              ...
              <div className="space-y-2 mb-6">
                <div className="text-gray-800 font-bold mb-2">Navigation</div>
                {navItems.map((item) => {
                  const isActive = location.pathname === item.path;
                  return (
                    <Link 
                    ...
                    >
                      <motion.div
                        ...
                        className={`flex items-center space-x-3 p-3 rounded-lg transition-colors ${
                          isActive 
                            ? (isProfileRoute ? 'bg-white/20 text-white' : 'bg-blue-50 text-blue-600')
                            : (isProfileRoute ? 'text-gray-300 hover:text-white hover:bg-white/10' : 'text-gray-800 hover:text-gray-900 hover:bg-gray-50')
                        }`}
                      >
                        <img 
                          src={item.icon} 
              ...
              {isAuthenticated && user ? (
                <div className={`mb-6 p-4 rounded-lg border ${
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    />
                    <div>
                      <p className={`font-semibold ${isProfileRoute ? 'text-white' : 'text-gray-900'}`}>
                        {user.display_name || user.username}
                      </p>
                      <p className={`text-sm ${isProfileRoute ? 'text-gray-300' : 'text-gray-500'}`}>
                        Level {calculateLevel(user?.total_xp || user?.xp || 0)} • {user.xp?.toLocaleString() || 0} XP
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center space-x-3 p-3 rounded-lg transition-colors ${
                      isProfileRoute ? 'text-red-400 hover:text-red-300 hover:bg-red-500/10' : 'text-red-600 hover:text-red-700 hover:bg-red-50'
                    }`}
                  >
                    <img 
                      src="/logout_24dp_E3E3E3_FILL0_wght400_GRAD0_opsz24.png" 
                      alt="Logout"
                      className="w-6 h-6"
                    />
