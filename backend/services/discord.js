...
/**
 * Send a project completion notification and schedule channel deletion
 */
const handleProjectCompletion = async (channelId, roleId, projectInfo, participants) => {
  try {
    ...
    const completionEmbed = {
      embeds: [{
        ...
        description: `**${projectInfo.project_title}** has been marked as complete!`,
        color: 0x00FF00, // Green
        fields: [
          {
            name: '👏 Congratulations Team!',
            value: `Amazing work everyone! ${participantMentions}`,
            inline: false
          },
          {
            name: '⚠️ Channel Closure Notice',
            value: '🗑️ This channel will be **deleted in 5 minutes** to keep the server organized.\n\n💾 **Save any important messages now!**',
            inline: false
          },
          {
            name: '🏆 Project Stats',
            value: `📅 **Completed:** ${new Date().toLocaleDateString()}\n👥 **Team Size:** ${participants.length} members\n⭐ **XP Earned:** ${projectInfo.xp_reward} XP each!`,
            inline: false
          }
        ],
        footer: {
          text: 'Thank you for using ProdigiousHub! 🚀',
        },
        timestamp: new Date().toISOString()
      }]
    };
    ...
    await channel.send({
      content: `🎊 **PROJECT COMPLETED!** 🎊\n${participantMentions}\n\n⏰ **IMPORTANT: This channel will be deleted in 5 minutes!**`,
      embeds: completionEmbed.embeds
    });

    console.log(`✅ Sent completion notification to channel ${channelId}`);

    // Schedule channel and role deletion after 5 minutes
    setTimeout(async () => {
      try {
        // Send final countdown
        await channel.send({
          content: '🚨 **FINAL WARNING: Channel deleting in 30 seconds!** 🚨\n\n💾 Save anything important NOW!',
          embeds: [{
            title: '⏰ Channel Deletion Imminent',
            description: 'This channel will be permanently deleted in 30 seconds.',
            color: 0xFF0000, // Red
            footer: {
        ...
        setTimeout(async () => {
          try {
            ...
            await role.delete('Project completed - cleaning up');
            console.log(`🗑️ Deleted role: ${role.name}`);

            // Delete the channel
            await channel.delete('Project completed - cleaning up');
            console.log(`🗑️ Deleted channel: ${channel.name}`);

          } catch (deleteError) {
            console.error('Error during final cleanup:', deleteError);
          }
        }, 30000); // 30 seconds

      } catch (warningError) {
        console.error('Error sending final warning:', warningError);
        // Still try to delete after the timeout
        setTimeout(async () => {
          try {
            await role.delete('Project completed - cleaning up');
            await channel.delete('Project completed - cleaning up');
    ...
    }, 4.5 * 60 * 1000); // 4.5 minutes (leaving 30 seconds for final warning)
...
/**
 * Send a welcome message to the project channel when a user joins
 */
const sendProjectWelcomeMessage = async (channelId, projectInfo) => {
  try {
    const channel = await client.channels.fetch(channelId);
    ...
    const welcomeMessage = {
      embeds: [{
        ...
        description: `Welcome <@${projectInfo.discord_user_id}> to the **${projectInfo.project_title}** project!`,
        color: 0x5865F2, // Discord blurple
        fields: [
          {
            name: '📋 Project Description',
            value: projectInfo.project_description || 'No description provided',
            inline: false
          },
          {
            name: '🔗 Project Link',
            value: `[View Project](${process.env.FRONTEND_URL}/dashboard/${projectInfo.project_slug})`,
            inline: false
          }
        ],
        footer: {
          text: 'ProdigiousHub Project Management',
          icon_url: 'https://cdn.discordapp.com/icons/your-server-id/icon.png' // You can add your server icon
        },
        timestamp: new Date().toISOString()
      }]
    };
    ...
    console.log(`✅ Sent welcome message to channel ${channelId} for user ${projectInfo.username}`);
