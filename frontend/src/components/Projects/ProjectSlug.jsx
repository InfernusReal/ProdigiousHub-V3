...
const ProjectSlug = () => {
  ...
  useEffect(() => {
    ...
    if (urlParams.get('discord') === 'success') {
      // Refresh Discord status when returning from OAuth
      setTimeout(() => {
        checkDiscordStatus();
      }, 1000); // Small delay to ensure backend has processed the OAuth
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 pt-16 sm:pt-20">
      <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-8">
        {/* Header */}
        <motion.div
        ...
        >
          <button
            onClick={() => navigate('/dashboard')}
            className="flex items-center gap-2 text-purple-300 hover:text-white mb-4 sm:mb-6 transition-colors text-sm sm:text-base"
          >
          ...
          {project.poster_image && (
            <div className="aspect-video sm:aspect-video md:aspect-[21/9] bg-gradient-to-r from-purple-500 to-blue-500 relative overflow-hidden rounded-lg sm:rounded-xl mb-4 sm:mb-8">
              <img
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-6 md:p-8">
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-bold text-white mb-2 sm:mb-4">{project.title}</h1>
                <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                  ...
                  <div className="flex items-center gap-1 sm:gap-2 text-white/90">
                    ...
                    <span className="text-xs sm:text-sm">{project.xp_reward} XP</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 lg:order-1 order-2">
            <motion.div
              ...
              className="bg-white/10 backdrop-blur-sm rounded-lg sm:rounded-xl border border-white/20 p-4 sm:p-6 lg:p-8 mb-4 sm:mb-8"
            >
              <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 sm:mb-6 gap-2 sm:gap-0">
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-white mb-2">About This Project</h2>
                  <p className="text-purple-200 text-sm sm:text-base">{project.description}</p>
                </div>
              </div>
                ...
                <div className="mb-4 sm:mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag, index) => (
                      <span
                        key={index}
                        className="px-2 sm:px-3 py-1 bg-purple-600/30 text-purple-200 rounded-full text-xs sm:text-sm"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              <AnimatePresence>
                {customContent?.sections && Array.isArray(customContent.sections) && customContent.sections.map((section, index) => (
                  <motion.div
                    ...
                    className="mb-6 sm:mb-8 p-4 sm:p-6 lg:p-8 bg-white/10 backdrop-blur-sm rounded-lg sm:rounded-xl border border-white/20"
                  >
                    {isCreator ? (
                      <div>
                        <input
                          ...
                          className="w-full mb-4 sm:mb-6 px-3 sm:px-4 py-2 sm:py-3 bg-white/10 border border-white/20 rounded-lg text-white text-lg sm:text-xl lg:text-2xl font-bold placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent caret-white"
                        ...
                        <textarea
                          ...
                          rows="6"
                          className="w-full mb-4 sm:mb-6 px-3 sm:px-4 py-2 sm:py-3 bg-white/10 border border-white/20 rounded-lg text-purple-200 placeholder-purple-300 resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent caret-white text-sm sm:text-base"
                      ...
                      <div>
                        <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-white mb-4 sm:mb-6">{section.title || 'Untitled Section'}</h2>
                        <p className="text-purple-200 leading-relaxed whitespace-pre-wrap text-sm sm:text-base">{section.content || 'No content'}</p>
                      </div>
                    )}
                  </motion.div>
                ))}
              ...
              {project.github_repo && (
                <div className="mt-4 sm:mt-6">
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-2 bg-gray-800 hover:bg-gray-700 text-white px-3 sm:px-4 py-2 rounded-lg transition-colors text-sm sm:text-base"
                  >
                    View on GitHub
                  </a>
                </div>
            ...
            <motion.div
              ...
              className="bg-white/10 backdrop-blur-sm rounded-lg sm:rounded-xl border border-white/20 p-4 sm:p-6 lg:p-8"
            >
              ...
              <div className="space-y-3 sm:space-y-4">
                  <div>
                    <textarea
                      ...
                      rows="3"
                      className="w-full px-3 sm:px-4 py-2 sm:py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-purple-300 resize-none focus:ring-2 focus:ring-purple-500 focus:border-transparent caret-white text-sm sm:text-base"
                    />
                    <button 
                      onClick={postComment}
                      disabled={isPostingComment || !newComment.trim()}
                      className="mt-2 sm:mt-3 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-3 sm:px-4 py-2 rounded-lg transition-colors flex items-center gap-2 text-sm sm:text-base"
                    >
                      {isPostingComment ? (
                        <>
                          <motion.div
                  ...
                  {project.comments && project.comments.map((comment, index) => (
                    <div key={comment.id} className="p-3 sm:p-4 bg-white/5 rounded-lg border border-white/10">
                      <div className="flex items-center gap-2 sm:gap-3 mb-2">
                        {comment.profile_picture ? (
                          ...
                          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center text-white text-xs font-bold">
                            {(comment.display_name || comment.username)?.charAt(0)?.toUpperCase()}
                          </div>
                        )}
                        <span className="text-white font-medium text-sm sm:text-base">{comment.display_name || comment.username}</span>
                        <span className="text-purple-300 text-xs sm:text-sm">{new Date(comment.created_at).toLocaleDateString()}</span>
                      </div>
                      <p className="text-purple-200 text-sm sm:text-base">{comment.content}</p>
                    </div>
                  ))}
                </div>
            </motion.div>
          ...
          <div className="lg:col-span-1 lg:order-2 order-1">
            <motion.div
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              className="bg-white/20 sm:bg-white/10 backdrop-blur-md sm:backdrop-blur-sm rounded-lg sm:rounded-xl border border-white/30 sm:border-white/20 p-4 sm:p-6 lg:sticky lg:top-24 mb-4 sm:mb-6 lg:mb-0 shadow-lg sm:shadow-none"
            >
              {/* Join Button */}
              {!isParticipant && !isCreator && project.status === 'open' && (
                <button
                  ...
                  disabled={joining}
                  className="w-full bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white font-bold py-3 sm:py-4 px-4 sm:px-6 rounded-lg flex items-center justify-center gap-2 mb-4 sm:mb-6 transition-colors text-sm sm:text-base"
                >
                  {joining ? (
                    <motion.div
                      animate={{ rotate: 360 }}
              ...
              {isParticipant && (
                <div className="w-full bg-green-600 text-white font-bold py-3 sm:py-4 px-4 sm:px-6 rounded-lg flex items-center justify-center gap-2 mb-4 sm:mb-6 text-sm sm:text-base">
                  <CheckIcon className="h-4 w-4 sm:h-5 sm:w-5" />
                  You're a member!
                </div>
              )}
              ...
              <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                <div className="flex items-center gap-2 sm:gap-3">
                  <UserGroupIcon className="h-4 w-4 sm:h-5 sm:w-5 text-purple-300" />
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <span className="text-white text-sm sm:text-base">{project.xp_reward} XP reward</span>
                </div>

                {project.deadline && (
                  <div className="flex items-center gap-2 sm:gap-3">
                    <CalendarIcon className="h-4 w-4 sm:h-5 sm:w-5 text-purple-300" />
                    <span className="text-white text-sm sm:text-base">
